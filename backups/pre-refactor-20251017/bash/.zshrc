...
# Fix for vscode/cursor IDE terminal issues
if [[ "$TERM_PROGRAM" == "vscode" ]] || [[ -n "$VSCODE_INJECTION" ]]; then
  # Unset problematic variables that cause issues with vscode/cursor's zsh integration
  unset argv 2>/dev/null || true
  # Force zsh to ignore errors from vscode/cursor's integration script
...
unsetopt correct_all # commands
unsetopt correct # words
...
if [[ "$TERM_PROGRAM" == "vscode" ]] || [[ -n "$VSCODE_INJECTION" ]]; then
  # Disable instant prompt completely in Cursor to avoid issues
...
# NOTE: Theme is disabled because we're using Powerlevel10k separately
ZSH_THEME=""
